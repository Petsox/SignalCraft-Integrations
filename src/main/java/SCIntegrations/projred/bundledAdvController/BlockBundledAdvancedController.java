...
public class BlockBundledAdvancedController extends BlockController {
    public BlockBundledAdvancedController(String name) {
        super(name);
    ...
    public void onNeighborBlockChange(final World world, final int x, final int y, final int z, final Block theBlock) {
        ...
        if (tileE instanceof TileBundledAdvancedController & !world.isRemote) {
            ((TileBundledAdvancedController) tileE).onNeighborUpdate();
        }
    }
    ...
    public boolean openGui(World world, int x, int y, int z, EntityPlayer player) {
        ...
        if (!(tileE instanceof TileController) || ((TileController) tileE).getGuiId() == null) {
            FMLLog.severe("SCIntegrations: BlockBundledAdvancedController.openGui found no valid TileBundledAdvancedController (with a set GuiID) at %d, %d, %d - tile was: %s",
                    x, y, z, tileE);
            return true;
        }
    ...
    public TileEntity createNewTileEntity(World world, int i) {
        return new TileBundledAdvancedController();
    }
}
