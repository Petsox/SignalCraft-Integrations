...
public class BlockBundledUniversalController extends BlockController {
    public BlockBundledUniversalController(String name) {
        super(name);
    ...
    public void onNeighborBlockChange(final World world, final int x, final int y, final int z, final Block theBlock) {
        ...
        if (tileE instanceof TileBundledUniversalController & !world.isRemote) {
            ((TileBundledUniversalController) tileE).onNeighborUpdate();
        }
    }
    ...
    public boolean openGui(World world, int x, int y, int z, EntityPlayer player) {
        ...
        if (!(tileE instanceof TileController) || ((TileController) tileE).getGuiId() == null) {
            FMLLog.severe("SCIntegrations: BlockBundledUniversalController.openGui found no valid TileBundledUniversalController (with a set GuiID) at %d, %d, %d - tile was: %s",
                    x, y, z, tileE);
            return true;
        }
    ...
    public TileEntity createNewTileEntity(World world, int i) {
        return new TileBundledUniversalController();
    }
}
