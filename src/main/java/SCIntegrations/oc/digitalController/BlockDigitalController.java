...
public class BlockDigitalController extends BlockController {
    public BlockDigitalController(String name) {
        super(name);
    ...
    public boolean openGui(World world, int x, int y, int z, EntityPlayer player) {
        ...
        if (!(tileE instanceof TileController)) {
            FMLLog.severe("SCIntegrations: BlockDigitalController.openGui found no valid TileDigitalController at %d, %d, %d - tile was: %s",
                    x, y, z, tileE);
            return true;
        }
    ...
    public TileEntity createNewTileEntity(World world, int i) {
        return new TileDigitalController();
    }
}
